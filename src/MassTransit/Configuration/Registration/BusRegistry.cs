namespace MassTransit.Registration
{
        ...
        IBusRegistry
    {
        ...
        public BusRegistry(IBusControl busControl, ILogger<BusRegistry> logger)
        {
            _logger = logger;
            _instances = new Dictionary<Type, IBusRegistryInstance>();

            var busRegistryInstance = new BusRegistryInstance(busControl);
            _instances.Add(busRegistryInstance.InstanceType, busRegistryInstance);
        }

        public BusRegistry(IBusControl busControl, IEnumerable<IBusRegistryInstance> instances, ILogger<BusRegistry> logger)
        {
            _logger = logger;
            _instances = instances.ToDictionary(x => x.InstanceType);
